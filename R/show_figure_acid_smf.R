...
edata <- select(edata, smf, plot, block, replanted, species, acidbrickRatioTreat, soilType, conf.low, conf.high)
...
pd <- position_dodge(.6)
ggplot(pdata, aes(acidbrickRatioTreat, smf, shape = acidbrickRatioTreat, ymin = conf.low, ymax = conf.high))+
  geom_quasirandom(data = edata, aes(acidbrickRatioTreat, smf), 
  ...
  labs(x = "", y = expression(Stem~mass~fraction~"("*SMF*")"~"["*g~g^-1*"]"), shape = "", color = "") +
  guides(x = guide_axis(angle = 45), shape = F)+
  themeMB()
ggsave("figure_acid_smf_(800dpi_8x7cm).tiff",
