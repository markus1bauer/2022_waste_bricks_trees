...
pd <- position_dodge(.6)
(srl <- ggplot(pdata, aes(soilType, srl, shape = brickRatio, ymin = conf.low, ymax = conf.high)) +
  ...
  labs(x = "Soil fertility", y = expression(Specific~root~length[1-3]~"["*m~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
  themeMB() +
