# Show Figure root-shoot ratio  ~ mycorrhiza:species:brickRatio ####
...
edata <- select(edata, rootshootRatio, plot, block, replanted, species, brickRatio, soilType, mycorrhiza, conf.low, conf.high)
...
pdata <- rename(pdata, rootshootRatio = predicted, mycorrhiza = x, brickRatio = group, species = facet)
meandata <- filter(pdata, mycorrhiza == "Control" & brickRatio == "5")
pd <- position_dodge(.6)
ggplot(pdata, aes(mycorrhiza, rootshootRatio, shape = brickRatio, ymin = conf.low, ymax = conf.high))+
  geom_quasirandom(data = edata, aes(mycorrhiza, rootshootRatio), 
                   color = "grey70", dodge.width = .6, size = 0.7)+
  geom_hline(aes(yintercept = rootshootRatio), meandata, color = "grey70") +
  geom_errorbar(position = pd, width = 0.0, size = 0.4) +
  geom_point(position = pd, size = 2.5) +
  facet_grid(~ species) +
  annotate("text", label = "n.s.", x = 2.2, y = 2) +
  ...
  themeMB()
ggsave("figure_mycorrhiza_rootshootRatio_(800dpi_12x6cm).tiff",
