...
pd <- position_dodge(.6)
(rmf <- ggplot(pdata, aes(soilType, rmf, shape = brickRatio, ymin = conf.low, ymax = conf.high)) +
  geom_quasirandom(data = data, aes(soilType, rmf), 
  ...
  labs(x = "Soil fertility", y = expression(Root~mass~fraction~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
  themeMB() +
...
#ggsave("figure_1_F_rmf_(800dpi_12x7cm).tiff",
