...
pd <- position_dodge(.6)
(lmf <- ggplot(pdata, aes(soilType, lmf, shape = brickRatio, ymin = conf.low, ymax = conf.high)) +
  ...
  labs(x = "Soil fertility", y = expression(Leaf~mass~fraction~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
  themeMB() +
