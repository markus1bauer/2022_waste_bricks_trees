...
(data <- select(data, lmf, plot, block, replanted, species, acid, brickRatio, acidbrickRatioTreat, soilType))
...
plot(lmf ~ acidbrickRatioTreat, data)
plot(lmf ~ block, data)
#2way (species:soilType):
ggplot(data, aes(soilType, lmf, color = species)) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#2way (species:replanted):
ggplot(data, aes(soilType, lmf, color = species)) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#3way (acidbrickRatioTreat:soilType):
ggplot(data, aes(acidbrickRatioTreat, lmf)) + facet_grid(~soilType) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#3way (brickRatio:acid:species):
ggplot(data, aes(acidbrickRatioTreat, lmf)) + facet_grid(~species) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#4way
ggplot(data, aes(soilType, lmf, color = acidbrickRatioTreat)) + facet_grid(~species) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#interactions with block:
ggplot(data, aes(species, lmf, color = acidbrickRatioTreat)) + geom_boxplot() + facet_wrap(~block) + geom_quasirandom(dodge.width = .7)
ggplot(data, aes(acidbrickRatioTreat, lmf)) + geom_boxplot() + facet_wrap(~block) + geom_quasirandom(dodge.width = .7)
...
dotchart((data$lmf), groups = factor(data$acidbrickRatioTreat), main = "Cleveland dotplot")
...
m1 <- lmer(lmf ~ species * acidbrickRatioTreat + (1|block), data, REML = F)
VarCorr(m1)
#3w-model
m2 <- lm(sqrt(lmf) ~ species * soilType * acidbrickRatioTreat, data)
simulateResiduals(m2, plot = T)
#full 2w-model
m3 <- lm(sqrt(lmf) ~ (species + soilType + acidbrickRatioTreat)^2, data)
