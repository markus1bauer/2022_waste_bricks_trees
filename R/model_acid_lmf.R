# Model for experiment acid and leaf mass fraction ####
...
(edata <- select(edata, lmf, plot, block, replanted, species, acid, brickRatio, acidbrickRatioTreat, soilType))
...
plot(lmf ~ acidbrickRatioTreat, edata)
plot(lmf ~ block, edata)
#2way (acidbrickRatioTreat):
ggplot(edata,aes(acidbrickRatioTreat, lmf)) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#2way (acidbrickRatioTreat:soilType):
ggplot(edata,aes(soilType, lmf, color = acidbrickRatioTreat)) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#3way (acidbrickRatioTreat:soilType):
ggplot(edata,aes(acidbrickRatioTreat, lmf)) + facet_grid(~soilType) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#3way (brickRatio:acid:species):
ggplot(edata,aes(acidbrickRatioTreat, lmf)) + facet_grid(~species) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#4way
ggplot(edata,aes(soilType, lmf, color = acidbrickRatioTreat)) + facet_grid(~species) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
# interactions with block:
ggplot(edata,aes(species, lmf, color = acidbrickRatioTreat)) + geom_boxplot() + facet_wrap(~block) + geom_quasirandom(dodge.width = .7)
ggplot(edata,aes(acidbrickRatioTreat, lmf)) + geom_boxplot() + facet_wrap(~block) + geom_quasirandom(dodge.width = .7)
...
dotchart((edata$lmf), groups = factor(edata$acid), main = "Cleveland dotplot")
par(mfrow=c(1,1));
boxplot(edata$lmf, ylim = c(0.0,0.2));#identify(rep(1,length(edata$lmf)),edata$lmf, labels = c(edata$no))
par(mfrow = c(2,2));
plot(table((edata$lmf)),type = "h", xlab = "Observed values", ylab = "Frequency")
plot(table(log(edata$lmf)), type = "h", xlab = "Observed values", ylab = "Frequency");
...
m1 <- lmer(lmf ~ species * acidbrickRatioTreat + (1|block), edata, REML = F)
VarCorr(m1)
#3w-model
m2 <- lm(sqrt(lmf) ~ species * soilType * acidbrickRatioTreat, edata)
simulationOutput <- simulateResiduals(m2, plot = T)
#full 2w-model
m3 <- lm(sqrt(lmf) ~ (species + soilType + acidbrickRatioTreat)^2, edata)
simulationOutput <- simulateResiduals(m3, plot = T)
#2w-model reduced
m4 <- lm(sqrt(lmf) ~ species + soilType + acidbrickRatioTreat +
...
### Model output ---------------------------------------------------------------------------------------------
m4 <- lm(lmf ~ species + soilType + acidbrickRatioTreat +
           acidbrickRatioTreat:species + acidbrickRatioTreat:soilType, edata)
summary(m4) #r2 = 0.783, r2a = 0.749
Anova(m4, type = 3)
