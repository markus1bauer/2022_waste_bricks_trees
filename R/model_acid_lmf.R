...
(edata <- read_table2("data_processed_acid.txt", col_names = T, na = "na", col_types = 
                        cols(
                          ...
                          date3 = col_date(),
                          species = col_factor(),
                          mycorrhiza = col_factor(levels = c("Control")),
...
ggplot(edata, aes(brickRatio, lmf, color = acid)) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#2way (brickRatio:soilType):
ggplot(edata, aes(soilType, lmf,color = brickRatio)) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#2way (soilType:acid):
ggplot(edata, aes(soilType, lmf, color = acid)) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#3way (brickRatio:acid:soilType):
ggplot(edata, aes(brickRatio, lmf, color = acid)) + facet_grid(~soilType) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#3way (brickRatio:acid:species):
ggplot(edata, aes(brickRatio, lmf, color = acid)) + facet_grid(~species) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#4way
ggplot(edata, aes(soilType, lmf, color = brickRatio, shape = acid)) + facet_grid(~species) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
# interactions with block:
ggplot(edata, aes(species, lmf, color = brickRatio, shape = acid)) + geom_boxplot() + facet_wrap(~block) + geom_quasirandom(dodge.width = .7)
ggplot(edata, aes(brickRatio, lmf, color = acid)) + geom_boxplot() + facet_wrap(~block) + geom_quasirandom(dodge.width = .7)
...
dotchart((edata$lmf), groups = factor(edata$watering), main = "Cleveland dotplot")
dotchart((edata$lmf), groups = factor(edata$seedmix), main = "Cleveland dotplot")
dotchart((edata$lmf), groups = factor(edata$grassRatio), main = "Cleveland dotplot")
par(mfrow=c(1,1));
boxplot(edata$lmf, ylim = c(0,45));#identify(rep(1,length(edata$lmf)),edata$lmf, labels = c(edata$no))
par(mfrow = c(2,2));
...
VarCorr(m1)
#4w-model
m2 <- lmer(lmf ~ species * soilType * brickRatio * acid +
             (1|block), edata, REML = F)
isSingular(m2)
simulationOutput <- simulateResiduals(m2, plot = T)
#full 3w-model
m3 <- lmer(lmf ~ (species + soilType + brickRatio + acid)^3 +
             (1|block), edata, REML = F)
isSingular(m3)
simulationOutput <- simulateResiduals(m3, plot = T)
#3w-model brick:water:mix
m4 <- lmer(log(lmf) ~ (brickRatio + acid + f.watering + seedmix) +
...
m5 <- lmer(log(lmf) ~ (brickRatio + acid + f.watering + seedmix) +  
             brickRatio:acid + brickRatio:f.watering + brickRatio:seedmix + 
...
#2w-model
m6 <- lmer(lmf ~ (species + soilType + brickRatio + acid) +
