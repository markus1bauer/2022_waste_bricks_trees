...
pd <- position_dodge(.6)
(sla <- ggplot(pdata, aes(soilType, sla, shape = brickRatio, ymin = conf.low, ymax = conf.high))+
  ...
  labs(x = "Soil fertility", y = expression(Specific~leaf~area~"["*cm^2~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
  themeMB() +
