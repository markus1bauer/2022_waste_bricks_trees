...
pdata <- rename(pdata, smf = predicted, soilType = x, brickRatio = group, species = facet)
meandata <- filter(pdata, soilType == "poor" & brickRatio == "5")
pd <- position_dodge(.6)
(smf <- ggplot(pdata, aes(soilType, smf, shape = brickRatio, ymin = conf.low, ymax = conf.high))+
  ...
  labs(x = "Soil fertility", y = expression(Stem~mass~fraction~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
  themeMB() +
...
#ggsave("figure_1_G_smf_(800dpi_12x7cm).tiff",
