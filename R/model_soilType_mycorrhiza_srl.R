...
par(mfrow=c(1,1));
boxplot(edata$srl, ylim = c(0.1,0.6));#identify(rep(1,length(edata$srl)),edata$srl, labels = c(edata$no))
par(mfrow = c(2,2));
...
#### a models ----------------------------------------------------------------------------------------
#random structure
m1 <- lmer(srl ~ species * brickRatio + (1|block), edata, REML = F)
VarCorr(m1)
#4w-model
m2 <- lmer(srl ~ species * brickRatio * soilType * mycorrhiza +
             (1|block), edata, REML = F)
isSingular(m2)
simulationOutput <- simulateResiduals(m2, plot = T)
#full 3w-model
m3 <- lmer(srl ~ (species + brickRatio + soilType + mycorrhiza)^3 +
             (1|block), edata, REML = F)
isSingular(m3)
...
#2w-model full
m5 <- lmer(srl ~ (species + brickRatio + soilType + mycorrhiza)^2 +
             (1|block), edata, REML = F)
isSingular(m5)
simulationOutput <- simulateResiduals(m5, plot = T)
#2w-model reduces
m6 <- lmer(srl ~ (species + brickRatio + soilType + mycorrhiza) +
             species:brickRatio + species:soilType + species:mycorrhiza +
             (1|block), edata, REML = F)
isSingular(m6)
simulationOutput <- simulateResiduals(m6, plot = T);
#1w-model full
m7 <- lmer(srl ~ (species + brickRatio + soilType + mycorrhiza) +
             (1|block), edata, REML = F)
isSingular(m7)
