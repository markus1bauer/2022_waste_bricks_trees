...
edata <- select(edata, leaf, sla, plot, block, replanted, species, brickRatio, mycorrhiza, soilType, conf.low, conf.high)
...
ggplot(pdata, aes(mycorrhiza, sla, shape = brickRatio, ymin = conf.low, ymax = conf.high))+
  ...
  labs(x = "", y = expression(Specific~leaf~area~"("*SLA*")"~"["*mg~mm^-2*"]"), shape = "Brick ratio [%]", color = "") +
