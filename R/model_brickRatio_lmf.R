...
VarCorr(m1)
#4w-model
m2 <- lmer(lmf ~ species * brickRatio * soilType * mycorrhiza + replanted +
             (1|block), edata, REML = F)
isSingular(m2)
simulationOutput <- simulateResiduals(m2, plot = T)
#full 3w-model
m3 <- lmer(lmf ~ (species + brickRatio + soilType + mycorrhiza)^3 + replanted +
             (1|block), edata, REML = F)
isSingular(m3)
simulationOutput <- simulateResiduals(m3, plot = T)
#3w-model brick:water:mix
m4 <- lmer(log(lmf) ~ (brickRatio + acid + f.watering + seedmix) + replanted +
...
m5 <- lmer(log(lmf) ~ (brickRatio + acid + f.watering + seedmix) + replanted + 
             brickRatio:acid + brickRatio:f.watering + brickRatio:seedmix + 
...
#2w-model
m6 <- lmer(log(lmf) ~ (brickRatio + acid + f.watering + seedmix) + replanted
             brickRatio:acid + brickRatio:f.watering + brickRatio:seedmix + 
             f.watering:seedmix + 
             (1|block), edata, REML = F)
isSingular(m6)
