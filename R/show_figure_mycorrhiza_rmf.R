...
edata <- select(edata, rmf, plot, block, replanted, species, brickRatio, soilType, mycorrhiza, conf.low, conf.high)
...
pdata <- rename(pdata, rmf = predicted, mycorrhiza = x, brickRatio = group, species = facet)
meandata <- filter(pdata, mycorrhiza == "Control" & brickRatio == "5")
pd <- position_dodge(.6)
ggplot(pdata, aes(mycorrhiza, rmf, shape = brickRatio, ymin = conf.low, ymax = conf.high))+
  geom_quasirandom(data = edata, aes(mycorrhiza, rmf), 
  ...
  labs(x = "", y = expression(Root~mass~fraction~"("*RMF*")"~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
  guides(x = guide_axis(angle = 0))+
  themeMB()
ggsave("figure_mycorrhiza_rmf_(800dpi_12x6cm).tiff",
