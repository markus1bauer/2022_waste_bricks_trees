...
# Model acid-treatment ~ root-shoot ratio####
# Markus Bauer
# 2022-03-15
...
(data <- read_csv2("data_processed_acid.csv",
    ...
    select(rootshootRatio, plot, block, replanted, species, acid, brickRatio,
...
ggplot(data, aes(soilType, rootshootRatio, color = species)) +
  geom_boxplot() + geom_quasirandom(dodge.width = .7)
#2way (species:replanted):
ggplot(data, aes(replanted, rootshootRatio, color = species)) +
  geom_boxplot() + geom_quasirandom(dodge.width = .7)
#3way (acidbrickRatioTreat:soilType):
ggplot(data, aes(acidbrickRatioTreat, rootshootRatio)) +
...
#4way
ggplot(data, aes(soilType, rootshootRatio, color = acidbrickRatioTreat)) +
  facet_grid(~species) + geom_boxplot() + geom_quasirandom(dodge.width = .7)
#interactions with block:
ggplot(data, aes(species, rootshootRatio, color = acidbrickRatioTreat)) +
  geom_boxplot() + facet_wrap(~block) + geom_quasirandom(dodge.width = .7)
ggplot(data, aes(acidbrickRatioTreat, rootshootRatio)) + geom_boxplot() +
...
m1 <- lmer((rootshootRatio) ~ species * acidbrickRatioTreat + (1 | block),
           data, REML = FALSE)
VarCorr(m1)
#3w-model
...
#full 2w-model
m3 <- lm(log(rootshootRatio) ~ (species + soilType + acidbrickRatioTreat)^2,
         data)
...
plotResiduals(main = "block",
              simulationOutput$scaledResiduals, data$block)
...
write.csv(tidytable, here("outputs", "statistics",
                          "table_anova_acid_rootshootRatio.csv"))
