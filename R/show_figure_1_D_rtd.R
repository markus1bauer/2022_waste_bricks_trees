...
pd <- position_dodge(.6)
(rtd <- ggplot(pdata, aes(soilType, rtd, shape = brickRatio, ymin = conf.low, ymax = conf.high)) +
  geom_quasirandom(data = data, aes(soilType, rtd), 
  ...
  labs(x = "Soil fertility", y = expression(Root~tissue~density[1-3]~"["*g~cm^-3*"]"), shape = "Brick ratio [%]", color = "") +
  themeMB() +
