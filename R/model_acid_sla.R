...
par(mfrow=c(1,1));
boxplot(edata$sla, ylim = c(100,400));#identify(rep(1,length(edata$sla)),edata$sla, labels = c(edata$no))
par(mfrow = c(2,2));
...
VarCorr(m1)
#4w-model
m2 <- lmer(log(sla) ~ species * soilType * brickRatio * acid +
             (1|block/plot), edata, REML = F)
isSingular(m2)
simulationOutput <- simulateResiduals(m2, plot = T)
#full 3w-model
m3 <- lmer(log(sla) ~ (species + soilType + brickRatio + acid)^3 +
             (1|block/plot), edata, REML = F)
isSingular(m3)
simulationOutput <- simulateResiduals(m3, plot = T)
#3w-model species:brickRatio:acid
m4 <- lmer(log(sla) ~ (species + soilType + brickRatio + acid)^2 +
             species:brickRatio:acid +
             (1|block/plot), edata, REML = F)
isSingular(m4)
simulationOutput <- simulateResiduals(m4, plot = T)
#3w-model species:brickRatio:soilType
m5 <- lmer(log(sla) ~ (species + soilType + brickRatio + acid)^2 +
             species:brickRatio:soilType + species:brickRatio:acid +
             (1|block/plot), edata, REML = F)
isSingular(m5)
simulationOutput <- simulateResiduals(m5, plot = T)
#2w-model
m6 <- lmer(log(sla) ~ (species + soilType + brickRatio + acid) +
...
### Effect sizes -----------------------------------------------------------------------------------------
(emm <- emmeans(m5, revpairwise ~ brickRatio * acid | species, type = "response"))
