# Show Figure relative growth rate ~ soilType:species:brickRatio ####
# Markus Bauer
...
pd <- position_dodge(.6)
(rgr13 <- ggplot(pdata, aes(soilType, rgr13, shape = brickRatio, ymin = conf.low, ymax = conf.high)) +
  ...
  labs(x = "Soil fertility", y = expression(paste("Relative growth rate")), shape = "Brick ratio [%]", color = "") +
  themeMB() +
