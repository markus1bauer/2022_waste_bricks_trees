...
(data <- read_csv("data_processed_brickRatio.csv",
    ...
    select(rootshootRatio, plot, block, replanted, species, brickRatio,
           soilType, mycorrhiza)
  )
...
ggplot(data, aes(soilType, rootshootRatio, color = brickRatio)) +
  geom_boxplot() + geom_quasirandom(dodge.width = .7)
#2way (brickRatio:mycorrhiza):
ggplot(data, aes(mycorrhiza, rootshootRatio, color = brickRatio)) +
  geom_boxplot() + geom_quasirandom(dodge.width = .7)
#2way (species:soilType):
ggplot(data, aes(species, rootshootRatio, color = soilType)) +
  geom_boxplot() + geom_quasirandom(dodge.width = .7)
#2way (species:mycorrhiza):
ggplot(data, aes(species, rootshootRatio, color = mycorrhiza)) +
  geom_boxplot() + geom_quasirandom(dodge.width = .7)
...
par(mfrow = c(1, 1));
boxplot(data$rootshootRatio, ylim = c(0.5,2.2))
plot(table((data$rootshootRatio)), type = "h",
...
#### a models ----------------------------------------------------------------
#random structure
m1 <- lmer(rootshootRatio ~ species * brickRatio + (1|block),
           data, REML = FALSE)
VarCorr(m1)
#4w-model
...
write.csv(tidytable, here("outputs", "statistics",
                          "table_anova_soilType_mycorrhiza_rootshootRatio.csv"))
