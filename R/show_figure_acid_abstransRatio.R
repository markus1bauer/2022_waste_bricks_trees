# Show Figure absorptive vs. transportive fine-root ratio ~ acid:brickRatio:species ####
...
edata <- select(edata, abstransRatio, plot, block, replanted, species, acidbrickRatioTreat, soilType, conf.low, conf.high)
...
pd <- position_dodge(.6)
ggplot(pdata, aes(acidbrickRatioTreat, abstransRatio, shape = acidbrickRatioTreat, ymin = conf.low, ymax = conf.high))+
  geom_quasirandom(data = edata, aes(acidbrickRatioTreat, abstransRatio), 
  ...
  labs(x = "", y = expression(Absorptive*":"*transport~fine~roots~"["*g~g^-1*"]"), shape = "", color = "") +
  guides(x = guide_axis(angle = 45), shape = F)+
  themeMB()
ggsave("figure_acid_abstransRatio_(800dpi_8x7cm).tiff",
