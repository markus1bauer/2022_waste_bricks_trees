# Show Figure stem mass fraction  ~ mycorrhiza:species:brickRatio ####
...
edata <- select(edata, smf, plot, block, replanted, species, brickRatio, soilType, mycorrhiza, conf.low, conf.high)
...
pd <- position_dodge(.6)
ggplot(pdata, aes(mycorrhiza, smf, shape = brickRatio, ymin = conf.low, ymax = conf.high))+
  geom_quasirandom(data = edata, aes(mycorrhiza, smf), 
                   color = "grey70", dodge.width = .6, size = 0.7)+
  geom_hline(aes(yintercept = smf), meandata, color = "grey70") +
  ...
  labs(x = "", y = expression(Stem~mass~fraction~"("*SMF*")"~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
  guides(x = guide_axis(angle = 0))+
  themeMB()
ggsave("figure_mycorrhiza_smf_(800dpi_12x6cm).tiff",
