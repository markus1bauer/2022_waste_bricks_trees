...
edata <- select(edata, srl, plot, block, replanted, species, brickRatio, soilType, mycorrhiza, conf.low, conf.high)

#### Chosen model ###
m4 <- lmer(srl ~ (species + brickRatio + soilType + mycorrhiza)^2 +
...
pdata <- rename(pdata, srl = predicted, mycorrhiza = x, brickRatio = group, species = facet)
meandata <- filter(pdata, mycorrhiza == "Control" & brickRatio == "5")
pd <- position_dodge(.6)
ggplot(pdata, aes(mycorrhiza, srl, shape = brickRatio, ymin = conf.low, ymax = conf.high))+
  geom_quasirandom(data = edata, aes(mycorrhiza, srl), 
                   color = "grey70", dodge.width = .6, size = 0.7)+
  geom_hline(aes(yintercept = srl), meandata, color = "grey70") +
  ...
  labs(x = "", y = expression(Specific~root~length~"("SRL*["1-3"]*")"~"["*m~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
  guides(x = guide_axis(angle = 0))+
  themeMB()
ggsave("figure_mycorrhiza_srl_(800dpi_12x6cm).tiff",
