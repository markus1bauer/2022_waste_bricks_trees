...
pd <- position_dodge(.6)
(abstransRatio <- ggplot(pdata, aes(soilType, abstransRatio, shape = brickRatio, ymin = conf.low, ymax = conf.high))+
  ...
  labs(x = "Soil fertility", y = expression(Absorptive*":"*transport~roots~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
  themeMB() +
