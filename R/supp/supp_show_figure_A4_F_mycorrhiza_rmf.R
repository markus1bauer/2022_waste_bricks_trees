...
(data <- select(data, rmf, plot, block, species, brickRatio, soilType, mycorrhiza, conf.low, conf.high))
...
pd <- position_dodge(.6)
(rmf <- ggplot(pdata, aes(mycorrhiza, rmf, shape = brickRatio, ymin = conf.low, ymax = conf.high)) +
    ...
    labs(x = "Mycorrhiza", y = expression(Root~mass~fraction~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
    themeMB() +
...
#ggsave("figure_A4_F_mycorrhiza_rmf_(800dpi_12x6cm).tiff",
