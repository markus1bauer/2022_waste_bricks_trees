...
(data <- select(data, lmf, plot, block, replanted, species, brickRatio, soilType, mycorrhiza, conf.high, conf.low))
...
pdata <- ggemmeans(m4, terms = c("mycorrhiza", "brickRatio", "species"), type = "fe", back.transform = T)
pdata <- rename(pdata, lmf = predicted, mycorrhiza = x, brickRatio = group, species = facet)
meandata <- filter(pdata, mycorrhiza == "Control" & brickRatio == "5")
pd <- position_dodge(.6)
(lmf <- ggplot(pdata, aes(mycorrhiza, lmf, shape = brickRatio, ymin = conf.low, ymax = conf.high)) +
    geom_quasirandom(data = data, aes(mycorrhiza, lmf), 
    ...
    labs(x = "Mycorrhiza", y = expression(Leaf~mass~fraction~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
    themeMB() +
...
#ggsave("figure_A4_E_mycorrhiza_lmf_(800dpi_12x6cm).tiff",
