...
pdata <- rename(pdata, lmf = predicted, mycorrhiza = x, brickRatio = group,
                species = facet)
meandata <- filter(pdata, mycorrhiza == "Control" & brickRatio == "5")
pd <- position_dodge(.6)

### plot ###
(lmf <- ggplot(pdata, aes(mycorrhiza, lmf, shape = brickRatio,
...
ggsave("figure_A4_E_mycorrhiza_lmf_800dpi_12x6cm.tiff",
