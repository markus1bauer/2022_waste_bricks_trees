...
(data <- read_csv("data_processed_acid.csv",
    ...
    select(srl, plot, block, species, acidbrickRatioTreat, soilType,
           conf.low, conf.high)
  )
...
pdata <- ggemmeans(m4, terms = c("acidbrickRatioTreat", "species"),
                   type = "fe")
pdata <- rename(pdata, srl = predicted, acidbrickRatioTreat = x,
                species = group)
...
### plot ###
(srl <- ggplot(pdata,
               aes(acidbrickRatioTreat, srl, shape = acidbrickRatioTreat,
...
ggsave("figure_A3_B_acid_srl_800dpi_8x7.5cm.tiff",
