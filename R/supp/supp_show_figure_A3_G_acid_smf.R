...
data <- select(data, smf, plot, block, species, acidbrickRatioTreat, soilType, conf.low, conf.high)
...
pdata <- rename(pdata, smf = predicted, acidbrickRatioTreat = x, species = group)
meandata <- filter(pdata, acidbrickRatioTreat == "Control 30% bricks")
pd <- position_dodge(.6)
(smf <- ggplot(pdata, aes(acidbrickRatioTreat, smf, shape = acidbrickRatioTreat, ymin = conf.low, ymax = conf.high))+
    geom_quasirandom(data = data, aes(acidbrickRatioTreat, smf), 
    ...
    labs(x = "", y = expression(Stem~mass~fraction~"["*g~g^-1*"]"), shape = "", color = "") +
    themeMB() +
...
#ggsave("figure_A3_G_acid_smf_(800dpi_8x7cm).tiff",
