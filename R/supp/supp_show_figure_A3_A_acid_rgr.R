...
data <- select(data, rgr13, plot, block, species, acidbrickRatioTreat, soilType, conf.low, conf.high)
...
pdata <- rename(pdata, rgr13 = predicted, acidbrickRatioTreat = x, species = group)
meandata <- filter(pdata, acidbrickRatioTreat == "Control 30% bricks")
pd <- position_dodge(.6)
(rgr13 <- ggplot(pdata, aes(acidbrickRatioTreat, rgr13, shape = acidbrickRatioTreat, ymin = conf.low, ymax = conf.high)) +
    ...
    facet_grid(~ species) +
    annotate("text", label = "n.s.", x = 3.2, y = 0.003) +
    scale_y_continuous(limits = c(0, .003), breaks = seq(-100, 100, 0.001)) +
    scale_shape_manual(values = c(1, 16, 15)) +
    labs(x = "", y = expression(paste("Relative growth rate")), shape = "") +
    themeMB() +
    theme(
...
#ggsave("figure_A3_A_acid_rgr_(800dpi_8x7cm).tiff",
