...
data <- select(data, rmf, plot, block, species, acidbrickRatioTreat, soilType, conf.low, conf.high)
...
pd <- position_dodge(.6)
(rmf <- ggplot(pdata, aes(acidbrickRatioTreat, rmf, shape = acidbrickRatioTreat, ymin = conf.low, ymax = conf.high))+
   geom_quasirandom(data = data, aes(acidbrickRatioTreat, rmf), 
    ...
    labs(x = "", y = expression(Root~mass~fraction~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
    themeMB() +
...
#ggsave("figure_A3_E_acid_rmf_(800dpi_8x7cm).tiff",
