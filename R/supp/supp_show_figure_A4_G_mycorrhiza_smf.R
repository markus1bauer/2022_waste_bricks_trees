# Show Figure root mass fraction  ~ mycorrhiza:species:brickRatio ####
# Markus Bauer
...
(data <- select(data, smf, plot, block, species, brickRatio, soilType, mycorrhiza, conf.low, conf.high))
...
pdata <- rename(pdata, smf = predicted, mycorrhiza = x, brickRatio = group, species = facet)
meandata <- filter(pdata, mycorrhiza == "Control" & brickRatio == "5")
pd <- position_dodge(.6)
(smf <- ggplot(pdata, aes(mycorrhiza, smf, shape = brickRatio, ymin = conf.low, ymax = conf.high))+
    ...
    labs(x = "Mycorrhiza", y = expression(Stem~mass~fraction~"["*g~g^-1*"]"), shape = "Brick ratio [%]", color = "") +
    themeMB() +
...
#ggsave("figure_A4_G_mycorrhiza_smf_(800dpi_12x6cm).tiff",
#       dpi = 800, width = 12, height = 6, units = "cm", path = "Z:/Documents/0_Ziegelprojekt/3_Aufnahmen_und_Ergebnisse/2020_waste_bricks_trees/outputs/figures/supp")
